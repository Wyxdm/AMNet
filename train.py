...
def get_model(args):
    ...
    optimizer_D = model.optimizer_D(model, args, LR=args.base_lr)
...
def main():
    ...
    model, optimizer, optimizer_D = get_model(args)
    if main_process():
        logger.info(model)
...
# ----------------------  TRAINVAL  ----------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # ----------------------  TRAIN  ----------------------
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, val_loader, model, optimizer, optimizer_D, epoch)
        ...
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            ...
            if osp.exists(filename):
                os.remove(filename)            
            torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(),'optimizer_D': optimizer_D.state_dict()}, filename)

        # -----------------------  VAL  -----------------------
        # if args.evaluate and epoch%1==0 and epoch>40:
        if args.evaluate and epoch%1==0:# and epoch>40:  
...
def train(train_loader, val_loader, model, optimizer, optimizer_D, epoch):
    global best_miou, best_FBiou, best_piou, best_epoch, keep_epoch, val_num
    global best_miou_m, best_miou_b, best_FBiou_m
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        lr_G = poly_learning_rate(optimizer, args.base_lr, current_iter, max_iter, power=args.power, index_split=args.index_split, warmup=args.warmup, warmup_step=len(train_loader)//2)
        lr_D = poly_learning_rate(optimizer_D, args.base_lr, current_iter, max_iter, power=args.power, index_split=args.index_split, warmup=args.warmup, warmup_step=len(train_loader)//2)
        ...
        output, main_loss, aux_loss1, aux_loss2, diversity_loss, loss_G, loss_D = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls)
        # print("GD",GD)
        if GD == 'G':
            # loss = main_loss + args.aux_weight1*aux_loss1 + args.aux_weight2*aux_loss2 + 0.1*loss_G
            # optimizer.zero_grad()
            # loss.backward()
            # optimizer.step()     
            if i%5 <= 3:
                loss = main_loss + args.aux_weight1*aux_loss1 + args.aux_weight2*aux_loss2 + 1e-9*loss_G + (loss_D + diversity_loss)*0
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            if (i%5 == 4) and loss_G <= 11:
                loss = main_loss + args.aux_weight1*aux_loss1 + args.aux_weight2*aux_loss2 + 0.1*loss_G + (loss_D + diversity_loss)*0
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()           
        
        if GD == 'D':
            loss = loss_D + 0.01*diversity_loss + (main_loss + aux_loss1 + aux_loss2 + loss_G)*0 
        ...
        diversity_loss_meter.update(diversity_loss.item(), n)
        G_loss_meter.update(loss_G.item(), n)
        D_loss_meter.update(loss_D.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'G_loss {G_loss_meter.val:.4f} '                        
                        'D_loss {D_loss_meter.val:.4f} '                        
                        'Diversity_loss {diversity_loss_meter.val:.4f} '                        
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch+1, args.epochs, i + 1, len(train_loader),
                                                        ...
                                                        G_loss_meter = G_loss_meter,
                                                        D_loss_meter = D_loss_meter,
                                                        diversity_loss_meter = diversity_loss_meter, 
            ...
            if args.viz:
                ...
                writer.add_scalar('loss_G', G_loss_meter.val, current_iter)
                writer.add_scalar('loss_D', D_loss_meter.val, current_iter)
                writer.add_scalar('diversity_loss', diversity_loss_meter.val, current_iter)
