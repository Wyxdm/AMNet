...
class MaskMultiHeadAttention(nn.Module):
    ...
    def scaled_dotproduct(self, query, key, value, mask=None, cross_att=False, tmp=0.1):
        ...
        if not cross_att:
            ...
            query, key = F.normalize(query, dim=2), F.normalize(key, dim=2) #norm on channel
            scores = torch.einsum("bmc,bnc->bmn", query, key) / tmp
        weight = scores

        if cross_att:
            attention = F.softmax(scores, dim=-2) # why softmax on the query dim
            attention = attention.masked_fill(mask == 0, 0) #将mask=0的地方填为0
